...
using SystemAgentInterop.Classes.Collections;
using SystemAgentInterop.Utils;

namespace SystemAgent.Management.Tasks
{
    public class TaskManager : ITaskManager
    {
        ...
        {
            // Annoying note - if there's an assembly in the Tasks DLL that isn't in the SystemCore
        ...
        public bool ProcessMessageResponse(MessageResponse resp)
        {
            ...
            if (resp.Responses != null && resp.Responses.Length > 0)
            {
                ...
                {
                    if (_agent.GetFileManager().GetPendingTransfers().Contains(t.SystemCoreTrackerUUID))
                    {
