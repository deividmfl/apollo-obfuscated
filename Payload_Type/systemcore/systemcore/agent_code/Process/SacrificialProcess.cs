...
using SystemAgentInterop.Features.WindowsTypesAndAPIs;
using SystemAgentInterop.Structs;
...
using AI = SystemInterop.Classes.Core;
...
namespace Process
{
    public class SacrificialProcess : AI.Process
    {
        ...
        public override bool StartWithCredentials(SystemCoreLogonInformation logonInfo)
        {
            bool bRet = false;
        ...
        public override bool StartWithCredentials(IntPtr hToken)
        {
            ...
            else
            {
                ...
                if (!bRet && (dwError == 1314)) // ERROR_PRIVILEGE_NOT_HELD or FILE_NOT_FOUND
                {
                    ...
                    {
                        if (_agent.GetIdentityManager().GetCurrentLogonInformation(out SystemCoreLogonInformation cred))
                        {
