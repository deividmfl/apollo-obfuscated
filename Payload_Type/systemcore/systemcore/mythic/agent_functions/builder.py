...
NOTE: v2.3.2+ has a different bof loader than 2.3.1 and are incompatible since their arguments are different
    ...
    agent_path = pathlib.Path(".") / "SystemCore" / "mythic"
    agent_code_path = pathlib.Path(".") / "SystemCore" / "agent_code"
    agent_icon_path = agent_path / "agent_functions" / "SystemCore.svg"
    build_steps = [
    ...
    async def build(self) -> BuildResponse:
        ...
        if self.get_parameter('debug'):
            possibleCommands = await SendMythicRPCCommandSearch(MythicRPCCommandSearchMessage(
                SearchPayloadTypeName="SystemCore",
            ))
        ...
        success_message = f"SystemCore {self.uuid} Successfully Built"
        stdout_err = ""
        ...
        for c2 in self.c2info:
            ...
            for key, val in c2.get_parameters_dict().items():
                ...
                if isinstance(val, dict) and 'enc_key' in val:
                    if val["value"] == "none":
                        resp.set_status(BuildStatus.Error)
                        resp.set_build_message("SystemCore does not support plaintext encryption")
                        return resp
        ...
        try:
            ...
            if stderr:
                stdout_err += f'[stderr]\n{stderr.decode()}' + "\n" + command
            output_path = f"{agent_build_path.name}/{buildPath}/SystemCore.exe"
            ...
            if os.path.exists(output_path):
                ...
                targetInteropPath = "/srv/SystemCoreInterop.dll"
                ...
                shutil.move(f"{agent_build_path.name}/{buildPath}/SystemCoreInterop.dll", targetInteropPath)
