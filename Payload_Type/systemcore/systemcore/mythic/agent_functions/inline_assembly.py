...
if platform.system() == 'Windows':
    INTEROP_ASSEMBLY_PATH = "C:\\Mythic\\SystemCore\\srv\\SystemCoreInterop.dll"
else:
    INTEROP_ASSEMBLY_PATH = "/srv/SystemCoreInterop.dll"
INTEROP_FILE_ID = ""
...
class InlineAssemblyCommand(CommandBase):
    ...
    async def build_interop(self):
        ...
        outputPath = "{}/SystemCoreInterop/bin/Release/SystemCoreInterop.dll".format(agent_build_path.name)
        copy_tree(str(self.agent_code_path), agent_build_path.name)
        shell_cmd = "dotnet build -c release -p:DebugType=None -p:DebugSymbols=false -p:Platform=x64 {}/SystemCoreInterop/SystemCoreInterop.csproj -o {}/SystemCoreInterop/bin/Release/".format(
            agent_build_path.name, agent_build_path.name)
        proc = await asyncio.create_subprocess_shell(shell_cmd, stdout=asyncio.subprocess.PIPE,
                                                     stderr=asyncio.subprocess.PIPE, cwd=agent_build_path.name)
        stdout, stderr = await proc.communicate()
        if not path.exists(outputPath):
            raise Exception("Failed to build SystemCoreInterop.dll:\n{}".format(stderr.decode() + "\n" + stdout.decode()))
    ...
    async def create_go_tasking(self, taskData: PTTaskMessageAllData) -> PTTaskCreateTaskingMessageResponse:
        ...
        if taskData.args.get_parameter_group_name() == "New Assembly":
            ...
            if len(searchedTaskResp.Tasks) == 0:
                # we need to register this file with SystemCore first
