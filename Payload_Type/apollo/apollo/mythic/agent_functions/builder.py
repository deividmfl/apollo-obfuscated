                    ...
                    if self.get_parameter('phantom_evasion') or self.get_parameter('phantom_crypter'):
                        try:
                            ...
                            if obfuscator_path.exists():
                                command = f"python3 {obfuscator_path}"
                                proc = await asyncio.create_subprocess_shell(command,
                                    stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE,
                                    cwd=agent_build_path.name)
                                stdout, stderr = await proc.communicate()
                                stdout_err += f'[Advanced Obfuscator]\n{stdout.decode()}\n{stderr.decode()}\n'
                            
                            # Step 3: Final stealth packing
                            packer_path = pathlib.Path(".") / "final_stealth_packer.py"
                            if packer_path.exists() and self.get_parameter('phantom_crypter'):
                                command = f"python3 {packer_path}"
                                proc = await asyncio.create_subprocess_shell(command,
                                    stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE,
                                    cwd=agent_build_path.name)
                                stdout, stderr = await proc.communicate()
                                stdout_err += f'[Final Stealth Packer]\n{stdout.decode()}\n{stderr.decode()}\n'
                                
                                # Check if stealth packed version was created
                                stealth_packed_path = f"{agent_build_path.name}/PhantomSecure.exe"
                                if os.path.exists(stealth_packed_path):
                                    final_payload_path = stealth_packed_path
                                    stdout_err += "[Phantom Crypter] Stealth packing successful\n"
